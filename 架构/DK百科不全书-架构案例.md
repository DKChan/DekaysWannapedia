...
flowchart TD
    ...
    subgraph 安全代理层
        ZTP[ZTP服务<br>Zero Touch Production]
        subgraph 安全控制
            ACL[ACL控制]
            AUDIT[审计日志]
            AUTH[身份认证]
    ...
    style ZTP fill:#e6f3ff
    style ACL fill:#fff4e6
...
flowchart LR
    ...
    style RM fill:#ffffcc
    style RH fill:#ffcccc
```

> 图：基于风险的访问分类，不同风险等级对应不同的控制强度
...
flowchart LR
    ...
    style TCB fill:#ccffcc
    style SB fill:#ffffcc
    style UA fill:#ffcccc
```

> 图：安全边界架构图，展示TCB可信计算基础与外部非信任区域的隔离关系
...
| 排名 | 漏洞类型 | 描述 | 防护措施 |
|:-----|:---------|:-----|:---------|
| 1 | 注入攻击 | SQL/NoSQL/OS命令注入 | 参数化查询、输入验证 |
| 2 | 失效的身份认证 | 会话管理缺陷、凭证泄露 | MFA、安全会话管理 |
| 3 | 敏感数据暴露 | 未加密传输/存储 | TLS、加密算法 |
| 4 | XML外部实体 | XXE攻击 | 禁用DTD、输入验证 |
| 5 | 失效的访问控制 | 越权访问 | 最小权限、ACL |
| 6 | 安全配置错误 | 默认配置、错误信息泄露 | 安全基线配置 |
| 7 | XSS跨站脚本 | 反射型/存储型XSS | 输出编码、CSP |
| 8 | 不安全的反序列化 | 远程代码执行 | 白名单、签名验证 |
| 9 | 使用含有漏洞的组件 | 依赖组件漏洞 | 组件扫描、及时更新 |
| 10 | 不足的日志监控 | 无法检测攻击 | 完善日志、实时监控 |

> 参考: OWASP Top 10 (Open Web Application Security Project)
...
flowchart TD
    ...
    D1 --> D2 --> D3 --> C1 --> C2 --> C3 --> P1 --> P2 --> P3 --> R1 --> R2 --> R3
...
flowchart LR
    ...
    style Ver fill:#ccffcc
    style Rev fill:#cce6ff
    style Imp fill:#cce6ff
```
