...
```mermaid
flowchart TD
    subgraph Phase1["1. 准备阶段"]
        P1["创建目标节点<br />cluster addslots"]
    end
    subgraph Phase2["2. 设置迁移状态"]
        P2A["源节点: cluster setslot MIGRATING"]
        P2B["目标节点: cluster setslot IMPORTING"]
    end
    subgraph Phase3["3. 数据搬迁"]
        P3A["获取keys: cluster getkeysinslot"]
        P3B["迁移: MIGRATE命令"]
    end
    subgraph Phase4["4. 完成迁移"]
        P4["所有节点: cluster setslot NODE"]
    end
    Phase1 --> Phase2
    Phase2 --> Phase3
    Phase3 --> Phase4
...
```mermaid
flowchart LR
    subgraph SingleThread["Redis单线程模型"]
        A["客户端请求"]
        B["事件循环"]
        C["命令处理"]
        D["返回结果"]
...
```mermaid
flowchart LR
    subgraph MultiThread["Redis 6.0+ 多线程模型"]
        A["客户端连接"]
        B["IO多线程"]
        C["主线程处理"]
        D["IO多线程返回"]
...
```mermaid
flowchart LR
    subgraph NetworkModel["Redis网络模型"]
        A["Socket连接"]
        B["AE事件循环"]
        C["文件事件"]
        D["时间事件"]
...
```mermaid
flowchart TD
    subgraph Startup["Redis启动流程"]
        A["main启动"]
        B["initServer"]
        C["创建事件循环aeCreateEventLoop"]
        D["监听端口"]
        E["注册时间事件"]
        F["aeMain事件循环"]
    end
...
```mermaid
flowchart LR
    subgraph QuickList["QuickList结构"]
        Head["Head Node"]
...
```mermaid
flowchart LR
    subgraph Listpack["Listpack结构"]
        A["编码类型<br />1-5 bytes"]
        B["元素数据<br />variable"]
        C["元素总长度<br />1-5 bytes"]
    end
    A --> B --> C
```
...
```mermaid
flowchart TD
    subgraph Encoding["Listpack编码类型"]
        A["整数编码"]
        B["字符串编码"]
        A1["int8"]
        A2["int16"]
...
```mermaid
flowchart LR
    subgraph Migrate["MIGRATE命令流程"]
        A["源实例"]
        B["目标实例"]
        C["DUMP数据"]
        D["RESTORE数据"]
    end
    A -->|"DUMP"| C -->|"传输"| D -->|"RESTORE"| B
```
...
```mermaid
flowchart LR
    subgraph TSDBArch["TSDB存储结构"]
        A["Chunks<br />压缩时序数据"]
        B["Index<br />倒排索引"]
        C["Meta.json<br />元信息"]
        D["Tombstones<br />删除标记"]
        A --> B
        B --> C
        C --> D
    end
...
```mermaid
flowchart LR
    subgraph ServiceDiscovery["服务发现架构"]
        A["服务提供者"]
        B["注册中心"]
        C["服务消费者"]
        D["健康检查"]
...
```mermaid
flowchart LR
    subgraph ProtocolCompare["实时协议对比"]
        A["WebSocket"]
        B["SSE"]
        C["长轮询"]
        D["MQTT"]
...
```mermaid
flowchart LR
    subgraph ProtocolFeature["协议特性对比"]
        A["协议"]
        B["传输方式"]
        C["实时性"]
        D["复杂度"]
...
```mermaid
flowchart TD
    subgraph ProtocolSelect["协议选择决策"]
        A{"需要双向通信?"}
        B{"需要低延迟?"}
        C{"IoT场景?"}
        D["WebSocket"]
