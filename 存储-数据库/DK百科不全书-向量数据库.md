...
<!-- 原图片: image3.png - Milvus系统架构图 -->
<!-- ❌ 未转换: 复杂系统架构图，包含多个组件和连接关系，文件大小311KB，字符画难以清晰表达 -->
<!-- 建议: 保留原图或使用专业绘图工具重新绘制 -->

**Milvus 架构概览** (文字描述):

```
Milvus 采用分布式微服务架构，主要组件包括:
...
┌───────────────────────────▼─────────────────────────────────┐
│                      存储层 (Storage)                         │
│  [对象存储 - MinIO/S3]  [元数据 - etcd]  [消息队列 - Pulsar]  │
└─────────────────────────────────────────────────────────────┘
```

*注: 详细架构请参考原图 image3.png 或 Milvus 官方文档*
