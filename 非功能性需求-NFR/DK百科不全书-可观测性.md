...
**流程说明**:
1. **明确目标** - 确定要调查的问题
2. **可视化发现** - 通过仪表盘查看telemetry数据，定位性能异常
3. **下钻分析** - 使用"宽事件"按属性分组/过滤，找出异常共性
4. **决策确认** - 是否已锁定根因维度？否 → 返回重新迭代
...
| 因素 | 系统 | 软件 |
|:---|:---|:---|
| **变化率** | 软件包更新（每月一次） | Repo 提交（每日） |
| **可预测性** | 高（稳定） | 低（许多新功能） |
| **对业务的价值** | 低（成本中心） | 高（创收） |
| **用户数量** | 很少（内部团队） | 许多（真正的客户） |
| **核心关注点** | 系统或服务是否健康 | 每个请求都能及时可靠地获得端到端执行所需的资源吗 |
| **评估角度** | 系统 | 真实用户 |
| **评估标准** | 底层内核和硬件设备驱动程序 | 变量和 API 端点 |
| **职能责任** | 基础设施运营 | 软件开发 |
| **理解方法** | 监控 | 可观测性 |
...
维度: HTTP方法(GET/POST) × 状态码(200/302/404/201/403/500)
```

**说明**: TSDB按维度基数相乘作为存储key，展示有限的基数和维度（HTTP方法和状态码标签，按时间戳分类）

高基数维度引起爆炸

<!-- 原图片: image4.png - 高基数维度爆炸示意图 -->
<!-- ❌ 未转换: 文件较大(189KB)，可能是复杂的数据增长/爆炸示意图，字符画难以清晰表达 -->
<!-- 建议: 保留原图 -->
...
┌─────────────────────────────────────────┐
│  维度: 方法(2) × 状态码(3) = 6 个时序     │
│  http.get.200, http.get.404, http.post.201 │
│  存储key数量: 6 × 时间桶数量              │
└─────────────────────────────────────────┘

高基数场景 (爆炸):
┌─────────────────────────────────────────┐
│  维度: 方法(2) × 状态码(3) × 用户ID(10万) │
│  × 路径(1000) × 地域(50)                │
│  存储key数量: 6 × 10万 × 1000 × 50      │
│            = 300亿 个时序!              │
│                                         │
│  结果: 存储爆炸、查询缓慢、内存耗尽       │
└─────────────────────────────────────────┘
