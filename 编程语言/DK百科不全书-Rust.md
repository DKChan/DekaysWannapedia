...
<!-- 原图片: image1.png - String内存布局图 -->
<!-- 已转换为字符画 -->

```
s (&String引用)          s1 (String)              堆内存
┌─────────┐          ┌─────────────┐        ┌─────┬─────┐
│  ptr    │─────────→│  ptr        │───────→│  0  │  h  │
└─────────┘          ├─────────────┤        ├─────┼─────┤
                     │  len: 5     │        │  1  │  e  │
...
**说明**: &String引用只包含一个指针(ptr)，指向String结构；String包含ptr/len/cap三个字段；实际数据存储在堆内存
...
<!-- 原图片: image2.png - 无限递归列表结构图 -->
<!-- 已转换为字符画 -->

```
无限递归的惰性列表 (Lazy Infinite List)

Cons⟨i32, Cons⟨i32, Cons⟨i32, Cons⟨i32, Cons⟨i32, ∞⟩⟩⟩⟩⟩

或 Lisp 风格表示:
(i32 . (i32 . (i32 . (i32 . (i32 . ∞)))))

每层 Cons = [数据值 i32 | 指向下一个 Cons]
                              ↓
                    无限循环回到自身 (∞)
```

<!-- 原图片: image3.png - Box解决递归类型大小问题 -->
<!-- 已转换为字符画 -->

```
使用 Box<T> 解决递归类型的无限大小问题
    ...
    │          │   Box    │
    │   i32    │ ┌──────┐ │
    │ (4字节)  │ │usize │ │  ← 指针(8字节)，而非直接嵌套List
    │          │ │指向下一节点│
    │          │ └──────┘ │
    └──────────┴──────────┘
    
    总大小固定: 4 + 8 = 12字节 (加上对齐为16字节)
    
    关键: Box是指针，大小已知，解决"递归类型具有无限大小"的编译错误
```
...
说明:
- 节点5被三个指针共同引用 (b→3→5, c→4→5, a→5)
- 两个链表在节点5处合并 (b链: 3→5→10→Nil, c链: 4→5→10→Nil)
- Rc<T>通过引用计数实现共享所有权
```
